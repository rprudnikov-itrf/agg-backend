...
{
    public static class СгенерироватьДоговор
    {
        ...
        public static Resource Run(AggregatorClient client, string reportPath, bool test = false)
        {
            ...
            var filename = string.Format("contract_{0}_{1:yyyyMMdd}.pdf", client.Contract.Number, client.Contract.Date);
            ...
            var fileName = @"contract_2017_08.xml";
            //var fileName = @"contract_2017_03.xml";
            //if (string.Equals(client.City, "Москва", StringComparison.CurrentCultureIgnoreCase))
            //    fileName = "contract_msk_2017_03.xml";

            var xtFile = Path.Combine(reportPath, "Resources", fileName);
            ...
            using (var file = new MemoryStream())
            {
                ...
                if (test)
                {
                    File.WriteAllBytes("e:\\contract.doc", file.ToArray());
                    return null;
                }
                else
                {
                    ...
                    AggregatorHelper.File.Add(client.Agg, client.Db, id, new AggregatorFile()
                    {
                        ...
                        Description = string.Format("Агентский договор №{0}-{1:yyyy}", client.Contract.Number, client.Contract.Date),
